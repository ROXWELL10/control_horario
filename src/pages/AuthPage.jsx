...
export default function AuthPage() {
  ...
  const handleSolicitarOtp = async () => {
    ...
    try {
      ...
      const destino = `+${res.telefono.telefono_whatsapp}`;
      if (res.modo === 'simulado') {
        setSuccessMsg(`Codigo OTP simulado para ${destino}: ${res.debugCodigo}`);
      } else {
        setSuccessMsg(`Codigo OTP enviado por WhatsApp a ${destino}.`);
      }
    } catch (err) {
  ...
  const handleRegister = async (e) => {
    ...
    if (!otpSolicitado) {
      setErrorMsg('Primero solicita el codigo OTP por WhatsApp.');
      return;
    }
            ...
            <form onSubmit={handleRegister} className="space-y-4 max-h-[420px] overflow-y-auto pr-1">
              ...
              <div>
                ...
                <div className="grid grid-cols-[118px_1fr] gap-3">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      inputMode="numeric"
                      value={celular}
                      onChange={(e) => { setCelular(e.target.value); resetOtpState(); }}
                ...
                <button
                  ...
                  className="mt-3 w-full py-3 bg-emerald-600/90 hover:bg-emerald-500 disabled:bg-emerald-600/40 text-white rounded-xl text-xs font-bold uppercase tracking-wider shadow-lg shadow-emerald-600/10 active:scale-95 transition-all flex items-center justify-center gap-2"
                >
                  <MessageCircle className="w-4 h-4" />
                  {otpEnviando ? 'Enviando codigo...' : otpSolicitado ? 'Reenviar codigo OTP' : 'Solicitar codigo OTP'}
                </button>
              </div>
